class Cube{
  ...
  public void turnX(int dir){
    ...
    for(int i = 0; i < size; i++){
      ...
      left_2[i] = Cube[(int(size * size) * (i + 1)) - 1 + off].getColorSet()[2];
      
      right_1[i] = Cube[int(size * size * i) + int(size - 1) + off].getColorSet()[4];
      right_2[i] = Cube[int(size * size * i) + int(size - 1) + off].getColorSet()[2];
      
      front_1[i] = Cube[int(size * size) + 1 - ((i * int(size)) + int(size - 1)) + off].getColorSet()[0];
      front_2[i] = Cube[int(size * size) + 1 - ((i * int(size)) + int(size - 1)) + off].getColorSet()[2];
      
      back_1[i] = Cube[((i * int(size)) + int(size - 1) + int(size * size * (size - 1))) + off].getColorSet()[1];
      back_2[i] = Cube[((i * int(size)) + int(size - 1) + int(size * size * (size - 1))) + off].getColorSet()[2];
    ...
    for(int i = 0; i < int(size); i++){
      ...
      Cube[((i * int(size)) + int(size - 1)) + off].setColorSet(2, left_2[i]);
      
      // Right Side
      Cube[((i * int(size)) + int(size - 1) + int(size * size * (size - 1))) + off].setColorSet(1, right_1[i]); 
      Cube[((i * int(size)) + int(size - 1) + int(size * size * (size - 1))) + off].setColorSet(2, right_2[i]); 
      
      // Front Side
      Cube[int(size * size * i) + int(size - 1) + off].setColorSet(4, front_1[i]);  
      Cube[int(size * size * i) + int(size - 1) + off].setColorSet(2, front_2[i]);  
      
      // Back Side
      Cube[int(size * size * (size + 1)) - (int(size * size) * (i + 1)) - 1 + off].setColorSet(5, back_1[i]);   
      Cube[int(size * size * (size + 1)) - (int(size * size) * (i + 1)) - 1 + off].setColorSet(2, back_2[i]);
  ...
  public void turnY(int dir){
    ...
    for(int i = 0; i < size; i++){
      ...
      left_2[i] = Cube[int(size - 1) - i + off].getColorSet()[4];
      
      right_1[i] = Cube[2 - i + int(size * size * (size - 1)) + off].getColorSet()[1];
      right_2[i] = Cube[2 - i + int(size * size * (size - 1)) + off].getColorSet()[4];
      
      front_1[i] = Cube[int(size * size * i) + int(size - 1) + off].getColorSet()[2];
      front_2[i] = Cube[int(size * size * i) + int(size - 1) + off].getColorSet()[4];
      
      back_1[i] = Cube[(i * int(size * size)) + off].getColorSet()[3];
      back_2[i] = Cube[(i * int(size * size)) + off].getColorSet()[4];
    }
    ...
    for(int i = 0; i < int(size); i++){
      ...
      Cube[(i * int(size * size)) + off].setColorSet(4, left_2[i]);
      
      //Right Side
      Cube[int(size - 1) + i * int(size * size) + off].setColorSet(2, right_1[i]);
      Cube[int(size - 1) + i * int(size * size) + off].setColorSet(4, right_2[i]);
      
      // Front Side
      Cube[i + off].setColorSet(0, front_1[i]); 
      Cube[i + off].setColorSet(4, front_2[i]); 
      
      // Back Side
      Cube[i + int(size * size * (size - 1)) + off].setColorSet(1, back_1[i]); 
      Cube[i + int(size * size * (size - 1)) + off].setColorSet(4, back_2[i]); 
   ...
   */
    ...
    for(int i = 0; i < size; i++){
      ...
      left_2[i] = Cube[(int((size - 1) * (size - 1) * (size - 1)) - i) + off].getColorSet()[0];
      
      right_1[i] = Cube[i + off].getColorSet()[4];
      right_2[i] = Cube[i + off].getColorSet()[0];
      
      front_1[i] = Cube[int(size * (size - 1)) - (i * int(size)) + off].getColorSet()[3];
      front_2[i] = Cube[int(size * (size - 1)) - (i * int(size)) + off].getColorSet()[0];
      
      back_1[i] = Cube[((i * int(size)) + int(size - 1)) + off].getColorSet()[2];
      back_2[i] = Cube[((i * int(size)) + int(size - 1)) + off].getColorSet()[0];
    }
    ...
    for(int i = 0; i < int(size); i++){
      ...
      Cube[((int(size - 1) - i) * int(size)) + off].setColorSet(0, left_2[i]);
      
      // Right Side
      Cube[((i * int(size)) + int(size - 1)) + off].setColorSet(2, right_1[i]);
      Cube[((i * int(size)) + int(size - 1)) + off].setColorSet(0, right_2[i]);
      
      // Front Side
      Cube[i + off].setColorSet(4, front_1[i]);   
      Cube[i + off].setColorSet(0, front_2[i]); 
      
      // Back Side
      Cube[(int((size - 1) * (size - 1) * (size - 1)) - i) + off].setColorSet(5, back_1[i]);
      Cube[(int((size - 1) * (size - 1) * (size - 1)) - i) + off].setColorSet(0, back_2[i]);
    }
